...
| 주문서 입력 | `/OAPI/V2/SaleOrder/SaveSaleOrder` | ✅ **전표 발행 성공** (`20260722-4`) |
| 판매 입력 | `/OAPI/V2/Sale/SaveSale` | ✅ **전표 발행 성공** (`20260722-24`, 2품목 → 1전표) |
| 거래처 등록 | `/OAPI/V2/AccountBasic/SaveBasicCust` | 🟡 엔드포인트 존재, 필드명 미확인 |

### 전표 발행 실측 규칙 (반드시 지킬 것)

1. **`UPLOAD_SER_NO`가 전표를 묶는 키다.**
   한 주문의 모든 품목 줄에 **같은 값**을 넣어야 전표 1건으로 등록된다.
   줄마다 다른 값을 넣으면 **품목 수만큼 전표가 쪼개진다.**
   (실측: 서로 다른 값 → 전표 2개 `20260722-6`,`-7` / 같은 값 → 전표 1개 `20260722-8`)
   짧은 값(`"1"`, `"21"`)은 정상 동작하고 8자리 값은 거부되므로, 주문번호를 이 필드에 넣지 말 것.
   주문 추적은 `REMARKS`에 기록한 내부 주문번호로 한다.

2. **이카운트는 중복 전송을 막지 않는다.**
   같은 내용·같은 주문번호로 다시 보내면 **새 전표가 하나 더 생성된다** (실측: `20260722-4` 재전송 → `20260722-5` 신규 생성).
   전표 조회 API도 이 회사에 열려 있지 않아 등록 여부를 되물을 수 없다.
   → **중복 방지는 전적으로 내부 큐의 `idempotency_key`와 "불명확하면 재전송 금지" 정책에 의존한다.**

3. **재시도 정책** (위 특성을 반영해 구현됨)
   | 실패 유형 | 처리 |
   |---|---|
   | 타임아웃·네트워크 오류 (성공 여부 불명확) | 재전송 안 함 → `MANUAL_REVIEW` |
   | 데이터 검증 오류 (미등록 코드 등) | 재시도 안 함 → `FAILED` (재시도해도 동일 실패, 오류 한도만 소모) |
   | 그 외 | 지수 백오프 재시도 |

4. **세션·호출 제한**: 짧은 시간에 반복 로그인하면 `Please login` → `service unavailable`로 차단된다.
   세션을 재사용하고, 큐 처리 간격을 두어야 한다.
