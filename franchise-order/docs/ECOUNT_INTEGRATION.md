...
4. **호출 제한 (이카운트 API인증현황 화면 고지)**
   ...
   **실서버 로그인은 10분에 1회**뿐이다. 매 요청마다 로그인하면 즉시 차단된다
   (`Please login` → `The service is unavailable`).

   구현된 대응:
   - 세션을 **프로세스 단위로 캐시**해 재사용 (`sessionCache`, TTL 30분)
   - 로그인 쿨다운을 코드에서 강제 — 초과 시 호출조차 하지 않고 남은 시간을 안내
   - 세션 만료(`Please login`) 감지 시 **1회만** 재로그인 후 재시도
   - 저장 호출은 **10초 간격 강제**(`throttleSave`)

   ⚠️ **처리량 한계**: 전표 발행은 10초에 1건이므로 **분당 최대 6건**이다.
   큐 배치 크기를 작게(기본 3건) 두고 크론을 1~2분 주기로 도는 방식을 쓴다.
   확정을 한꺼번에 많이 하면 전송이 순차적으로 밀린다(주문 30건 ≈ 5분).
